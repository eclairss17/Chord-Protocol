        ...
        actor {
            ...
            | SearchKey (keyid)-> 
                ...
                if(keys.Contains(keyid)) then 
                    ...
                    printf "\n-------------suc%i ----------------------------------------\n" successor.Count 
                    for KeyValue(k,v) in successor do
                        printfn "Print inside for k---------->%i" k
                        if k<=keyid then 
                            temp1 <- k                        
...
let Chordinitiate (totalNodes: int) =
    ...
    // let mutable requests= int(numberOfRequests)
    ...
    for KeyValue(k,v) in chordMap do
        ...
        if (chordMap.ContainsKey(nodeselect)) then 
            ...
            response3<- Async.RunSynchronously (chordMap.[nodeselect] <? SearchKey(findKey))            
            if (response3 = 1) then
